...
const getVideoSignature = async (req, res) => {
  try {
    ...
    const existingVideo = await SolutionVideo.findOne({ problem: problemId });

    // 🔹 allow reupload if updating
    if (existingVideo) {
      console.log("Existing video found. Generating new signature for update.");
    }

    // generate new unique public_id for every upload (new or update)
    const timestamp = Math.round(new Date().getTime() / 1000);
    ...
    const reply = {
      ...
      message: existingVideo 
        ? 'Existing video found — signature generated for reupload.'
        : 'New video signature generated.',
    };
