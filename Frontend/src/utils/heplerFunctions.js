...
export function summarizeSubmissionsByDate(submissions) {
  ...
  for (const item of submissions) {
    if (!item.isSolved || !item.pid || !item.pid.difficulty) continue; // ✅ Skip unsolved items or items with missing pid
...
export const getSolvedStats = (checkedProblems) => {
  ...
  for (const problem of checkedProblems) {
    if(problem.isSolved && problem.pid && problem.pid.difficulty) {
