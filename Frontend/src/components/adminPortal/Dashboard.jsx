...
const ActionCard = ({ to, title, description, icon }) => (
  <Link
    to={to}
    className="block bg-[#FFFFFF] dark:bg-[#1E293B] p-6 rounded-xl border border-[#E2E8F0] dark:border-[#334155] hover:border-[#3B82F6] dark:hover:border-[#3B82F6] transition-all duration-300 transform hover:-translate-y-1 hover:shadow-lg"
  >
    <div className="flex items-center space-x-4 mb-2">
      <div className="p-3 bg-[#3B82F6]/10 dark:bg-[#3B82F6]/20 rounded-lg text-[#3B82F6]">
        {icon}
      </div>
          ...
          <div className="bg-[#FFFFFF] dark:bg-[#1E293B] p-4 rounded-xl border border-[#E2E8F0] dark:border-[#334155] space-y-2">
            {
              data.latestSubmissions?.map((submission) => 
                <RecentSubmissionItem key={submission._id} title={submission.problemId?.title || "Unknown Problem"} user={submission.userId?.username || "Unknown User"} status={getProcessedStatus(submission.status)} time={getTimeStamp(submission.createdAt)} />
              )
            }
          </div>
        </div>
