    ...
    <div className="h-full w-full flex flex-col">
      ...
      <div className="h-[42.4px] flex items-center justify-between px-3 bg-gray-100 dark:bg-slate-900 border-b border-gray-300 dark:border-slate-700 text-xs text-gray-600 dark:text-slate-300 flex-shrink-0">
        ...
        <div className="flex items-center space-x-2">
          ...
          {showSubmissionResult&&!submissionResult ? (
            <button onClick={handleSubmit} className="px-3 py-1 text-xs bg-blue-400 text-white rounded opacity-80 transition-colors cursor-not-allowed" disabled={true}>Submitting</button>
          ) : (
          <button onClick={handleSubmit} className="px-3 py-1 text-xs bg-blue-500 text-white rounded hover:opacity-90 transition-colors cursor-pointer" >Submit</button>
          )}
          <div className='relative'>
      ...
      {!isAuthenticated && (
          <div className="h-10 p-2.5 bg-gray-100 dark:bg-slate-700 text-sm text-center border-y border-gray-300 dark:border-slate-700 text-gray-800 dark:text-slate-100 flex-shrink-0">
            You need to <span onClick={()=>{navigate("/login")}} className="text-blue-500 font-semibold hover:underline cursor-pointer">Login</span> / <span onClick={()=>{navigate("/login")}} className="text-blue-500 font-semibold hover:underline cursor-pointer">Signup</span> to run or submit
          </div>
      )}
