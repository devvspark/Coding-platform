              ...
              {bottomTabs.map(tab => (
                <button
                  ...
                  className={`py-2 px-4 text-xs font-medium focus:outline-none ${
                    activeTab === tab
                      ? 'text-blue-500 border-b-2 border-blue-500 bg-white dark:bg-slate-800'
                ...
                <div className="space-y-6 text-gray-800 dark:text-slate-100">
                  <div className="flex mt-2 space-x-2">
                    { testCases.map((_, index) => 
                        <button key={`case${index+1}`} onClick={() => {setActiveCase(index+1)}} className={`text-xs px-2 py-1 rounded bg-gray-200 dark:bg-slate-700 hover:bg-gray-300 border ${activeCase===index+1?"border-blue-500 text-blue-500":"border-gray-400"} dark:hover:bg-slate-600`}>{`Case ${index+1}`}</button>
                    )}
                  </div>
