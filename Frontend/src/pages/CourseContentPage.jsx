            ...
            {mockDiscussions.map((thread, index) => (
              <div key={thread.id} className={`p-4 bg-slate-100 dark:bg-slate-800 rounded-lg shadow ${index < mockDiscussions.length -1 ? "border-b border-slate-200 dark:border-slate-700 pb-6" : ""}`}>
                <div className="flex items-start space-x-3">
                  ...
                  <div className="flex-1">
                    <div className="flex items-center justify-between">
                      <span className="text-sm font-semibold text-blue-500 dark:text-blue-400">{thread.user.name}</span>
                ...
                {thread.replies && thread.replies.length > 0 && (
                  <div className="ml-10 mt-4 pl-4 border-l border-slate-300 dark:border-slate-700 space-y-4">
                    {thread.replies.map(reply => (
                       <div key={reply.id} className="flex items-start space-x-3">
                          ...
                          <div className="flex-1">
                            <div className="flex items-center justify-between">
                              <span className="text-xs font-semibold text-blue-500 dark:text-blue-300">{reply.user.name}</span>
                 ...
                 <button className="mt-4 text-xs text-blue-600 hover:text-blue-500 dark:text-blue-500 dark:hover:text-blue-400 hover:underline">Reply to thread</button>
              </div>
            ))}
          </div>
          <div className="mt-8 pt-6 border-t border-slate-200 dark:border-slate-700">
              <textarea
                  className="w-full p-3 bg-white dark:bg-slate-700 text-slate-800 dark:text-slate-200 rounded-md border border-slate-300 dark:border-slate-600 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 placeholder-slate-400 dark:placeholder-slate-400"
                  rows={3}
              ...
              {lecture.attachments.map((att, index) => (
                <li key={index} className="bg-slate-100 dark:bg-slate-800 p-3 rounded-md flex items-center justify-between hover:bg-slate-200 dark:hover:bg-slate-700 transition-colors group">
                  <div className="flex items-center">
                    <PaperClipIcon className="w-5 h-5 text-blue-500 dark:text-blue-400 mr-3 flex-shrink-0 group-hover:text-blue-400 dark:group-hover:text-blue-300"/>
                    <div>
                  ...
                  <a href={att.url} download className="text-sm text-blue-600 hover:text-blue-500 dark:text-blue-500 dark:hover:text-blue-400 font-medium p-2 rounded-md hover:bg-slate-300/50 dark:hover:bg-slate-750">Download</a>
                </li>
              ))}
            </ul>
          ) : (
          ...
          <div className="flex items-center justify-between">
            <div>
                ...
                <p className="text-xs text-slate-500 dark:text-slate-400">
                    Part of: <Link to={APP_ROUTES.courseOverview.replace(':courseId', course.id)} className="text-blue-500 dark:text-blue-400 hover:underline hover:text-blue-600 dark:hover:text-blue-300">{course.snippet?.title}</Link>
                </p>
            </div>
            <button
              onClick={() => setIsCurriculumSidebarVisible(!isCurriculumSidebarVisible)}
              className="md:hidden p-2 text-slate-500 dark:text-slate-300 hover:text-slate-800 dark:hover:text-white focus:outline-none focus:ring-2 focus:ring-inset focus:ring-blue-500 rounded-md"
                ...
                {TABS_CONFIG.map((tab) => (
                  <button
                    ...
                    className={`whitespace-nowrap py-3 px-2 sm:px-3 border-b-2 font-medium text-sm flex items-center transition-all rounded-t-md
                      ${activeTab === tab.id
                        ? 'border-blue-500 text-blue-500 bg-blue-500/10 dark:bg-slate-800'
                  ...
                  >
                    <tab.icon className={`w-5 h-5 mr-1.5 sm:mr-2 ${activeTab === tab.id ? 'text-blue-500' : 'text-slate-400 dark:text-slate-500 group-hover:text-slate-500 dark:group-hover:text-slate-400'}`} />
                    {tab.name}
                  </button>
                ))}
              </nav>
      ...
      `}>
        <div className="p-4 border-b border-slate-200 dark:border-slate-700 flex-shrink-0 sticky top-0 bg-slate-50 dark:bg-slate-800 z-10">
            <div className="flex items-center justify-between mb-2">
                <Link to={APP_ROUTES.courseOverview.replace(':courseId', course.id)} className="flex items-center text-blue-500 hover:text-blue-600 dark:hover:text-blue-300 group transition-colors">
                    <BookOpenIcon className="w-5 h-5 mr-2 group-hover:text-blue-600 dark:group-hover:text-blue-300"/>
                    <h2 className="text-md font-semibold truncate group-hover:text-blue-600 dark:group-hover:text-blue-300">{course.title}</h2>
                </Link>
                <button
                    onClick={() => setIsCurriculumSidebarVisible(false)}
                    className="md:hidden text-slate-500 dark:text-slate-400 hover:text-slate-800 dark:hover:text-white p-1 focus:outline-none focus:ring-2 focus:ring-inset focus:ring-blue-500 rounded-md"
          ...
          {course.curriculum.map((module) => (
            <div key={module.id} className="mb-1">
              <button
                ...
                className="w-full flex justify-between items-center px-4 py-2.5 text-sm font-medium text-left text-slate-700 dark:text-slate-300 hover:bg-slate-200 dark:hover:bg-slate-700 focus:outline-none focus:bg-slate-200 dark:focus:bg-slate-700 focus:ring-1 focus:ring-blue-500 transition-colors rounded-md mx-2"
                      ...
                      <button
                        ...
                        className={`w-full text-left px-3 py-2 text-xs rounded-md transition-all my-0.5 group ${
                          lecture.contentDetails?.videoId === currentLecture.contentDetails?.videoId
                            ? 'bg-blue-600 text-white font-semibold shadow-md ring-1 ring-blue-400'
                            : 'text-slate-600 dark:text-slate-300 hover:bg-slate-200 dark:hover:bg-slate-700 hover:text-blue-500 dark:hover:text-blue-400'
                        }`}
                      ...
                      >
                        <div className="flex items-center">
                          {completedLectures[lecture.contentDetails?.videoId] ? (
                            ...
                            <VideoCameraIcon className={`w-4 h-4 mr-2 flex-shrink-0 ${lecture.contentDetails?.videoId === currentLecture.contentDetails?.videoId ? 'text-white' : 'text-slate-500 group-hover:text-blue-500 dark:text-slate-400 dark:group-hover:text-blue-400'}`} />
                          )}
