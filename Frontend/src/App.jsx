...
import { AllRecentSubmissionsPage, Dashboard, ProblemManager, CreateProblem, UpdateProblem, CreateUser, UserManager, UpdateUser, VideoSolutionManager, UploadVideoSolution, SprintDetail, EditVideoSolution } from './components';
    ...
    <div className={`${darkTheme? "dark": ""} flex flex-col min-h-screen bg-white dark:bg-gray-900`}> 
      {loading ? 
        ...
        <Routes>
          ...
          <Route path="/admin-portal" element={<AdminPortal darkTheme={darkTheme} handleThemeChange={toggleTheme} />} >
            ...
            <Route path="video-solutions/edit/:ProblemId" element={<EditVideoSolution />} />
          </Route>
          <Route path="/problems" element={ isAuthenticated ? <ProblemsPage darkTheme={darkTheme} handleThemeChange={toggleTheme} /> : <Navigate to="/login" /> }>
            <Route path=":problemId" element={ <SolveProblemPage darkTheme={darkTheme} handleThemeChange={toggleTheme} /> }/>
          </Route>
        </Routes>
      }
    </div>
